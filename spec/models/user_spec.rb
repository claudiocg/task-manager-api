...
RSpec.describe User, type: :model do
  ...
  it { is_expected.to allow_value('claudio@claudio.com').for(:email) }
  ...
  describe '#generate_authentication_token!' do
    it 'generates a unique ath token' do
      allow(Devise).to receive(:friendly_token).and_return('abc123xyzTOKEN')
    ...
    it 'generates another auth token when the current auth token already has been taken' do
      allow(Devise).to receive(:friendly_token).and_return('abc123tokenxyz', 'abc123tokenxyz', 'abcZYZ123456789')
